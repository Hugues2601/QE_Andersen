...
class EnhancedGreekLearningDataset(Dataset):
...
class EnhancedGreekPredictorMLP(nn.Module):
    def __init__(self, dropout_rate=0.2):
        super(EnhancedGreekPredictorMLP, self).__init__()
        ...
        # Couche de sortie pour les 5 coefficients de correction des grecs
...
def compute_pnl_explained(predicted_greeks, features, dt=1 / 252):
    """
    Recalcule le PnL expliqué avec les grecs prédits par le modèle.

    Args:
        predicted_greeks : (batch_size, 5) coefficients prédits par le MLP
    ...
    # Extrait les coefficients de correction des grecs
    delta_coeff = predicted_greeks[:, 0]
    vega_coeff = predicted_greeks[:, 1]
    theta_coeff = predicted_greeks[:, 2]
    vanna_coeff = predicted_greeks[:, 3]
    volga_coeff = predicted_greeks[:, 4]

    # Calcule le PnL expliqué
    pnl_explained = (
            delta_coeff * delta_S +
            vega_coeff * delta_sqrt_v +
            theta_coeff * dt +
            vanna_coeff * delta_S * delta_sqrt_v +
            0.5 * volga_coeff * (delta_sqrt_v ** 2)
    )
...
def pnl_loss(predicted_greeks, features, pnl_tot, dt=1 / 252, l1_lambda=1e-5):
    ...
    Args:
        predicted_greeks : (batch_size, 5) coefficients prédits
    ...
    # Régularisation L1 (pour encourager la parcimonie des coefficients)
...
def train_model(model, train_loader, val_loader=None, epochs=100, lr=5e-4, weight_decay=1e-4,
    ...
    """
    Fonction d'entraînement améliorée avec validation précoce et learning rate scheduling
...
def evaluate_model(model, test_loader, forward_model, normalization_params=None,
                   device='cuda' if torch.cuda.is_available() else 'cpu'):
    """
    Évalue le modèle et calcule les grecs corrigés

    Args:
        model : modèle entraîné
        test_loader : DataLoader pour les données de test
        forward_model : modèle de pricing pour obtenir les grecs de base
    ...
    all_coeffs = torch.cat(all_coeffs).numpy()
    ...
    # Extraire les coefficients de correction
    a_pred = all_coeffs[:, 0]
    b_pred = all_coeffs[:, 1]
    c_pred = all_coeffs[:, 2]
    d_pred = all_coeffs[:, 3]
    ...
    results = {
        ...
        'greeks': {
            ...
            'vanna_corr': vanna_corr,
            'volga_corr': volga_corr
        },
        'pnl': {
            'real': all_real_pnl,
...
if __name__ == "__main__":
    # Supposons que tout le code précédent est disponible
    import torch
    import numpy as np
    ...
    train_dataset = EnhancedGreekLearningDataset(
    ...
    val_dataset = EnhancedGreekLearningDataset(
    ...
    model = EnhancedGreekPredictorMLP(dropout_rate=0.2)
    ...
    }, "enhanced_greek_model_complete.pth")
    ...
    # Extraction des snapshots
    St_test, St1_test, vt_test, vt1_test = extract_snapshots(S_test, v_test, t=50)
    ...
    test_dataset = EnhancedGreekLearningDataset(
    ...
    # Afficher quelques exemples de grecs corrigés
    print("\nExemples de grecs corrigés (chemin 0 à 9):")
    for i in range(10):
        print(
            f"Chemin {i}: "
            f"Δ={results['greeks']['delta_corr'][i]:.4f}, "
            f"ν={results['greeks']['vega_corr'][i]:.4f}, "
            f"θ={results['greeks']['theta_corr'][i]:.6f}, "
            f"Vanna={results['greeks']['vanna_corr'][i]:.6f}, "
            f"Volga={results['greeks']['volga_corr'][i]:.4f}"
        )

    # Distribtion des coefficients
    import matplotlib.pyplot as plt

    fig, axes = plt.subplots(2, 3, figsize=(15, 10))
    axes = axes.flatten()

    coeff_names = ['delta_coeff', 'vega_coeff', 'theta_coeff', 'vanna_coeff', 'volga_coeff']

    for i, name in enumerate(coeff_names):
        axes[i].hist(results['coefficients'][name], bins=50, alpha=0.7)
        axes[i].set_title(f'Distribution de {name}')
        axes[i].set_xlabel('Valeur du coefficient')
    ...
    plt.tight_layout()
    plt.savefig('greek_correction_results.png')
